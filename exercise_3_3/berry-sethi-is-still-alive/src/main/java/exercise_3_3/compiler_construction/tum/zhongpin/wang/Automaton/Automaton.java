...
public class Automaton {
    ...
    public Automaton(boolean hasPartialDFA) {
        this(new ArrayList<>(), hasPartialDFA);
    }

    public Automaton(List<State> states, boolean hasPartialDFA) {
        this.states = states;
        if (hasPartialDFA) {
            partialDFA = new Automaton(false);
        } else {
            partialDFA = null;
        }
    }
    ...
    public void next(char c) {
        if (partialDFA == null) {
            ...
            // // This automaton is a NFA without DFA
            // if (partialDFA == null) {
    ...
    public void updateCurrentState() {
        if (partialDFA != null) {
            ...
            for (String currentStateNFAName: currentStateNFANames) {
                this.currentStates.add(states.stream().filter(s -> s.getName().equals(currentStateNFAName)).findFirst().get());
            }
        }
    }
